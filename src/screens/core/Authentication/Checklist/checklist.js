...
import { View, TouchableOpacity, SafeAreaView, Text, StyleSheet, FlatList, ActivityIndicator, Image,  } from 'react-native';
...
export default function Agendamentos({ navigation }) {
    ...
    listAgendamento = async (data, token, emitente, start,max,coluna,valor, sortColumnIndex, sortDirection) => {
        ...
        var obj={"id":data.id ? data.id : 0 ,"secho":"","startRow":start,"maxResults":10,"filtros":[],"programa":"ORD600"}
        if(data){
    ...
    onselectButton = (id,item) => {
        SelectedBox();
        console.log("select btn",id)
        // console.log("seleelele",seletedbox)
        SelectedBox(id);
        setTickbtn(false);
    ...
    nextbtn = async () => {
        console.log("next bcounttt",count)
        setCount(count + 10)
        var start = count + 10;
        console.log("countttttttttttttt",start)
        if (netInfo.isConnected == true) {
            const token = await AsyncStorage.getItem('token');
            const emitente = await AsyncStorage.getItem('codigoEmitente');
            setProgress(true);
    ...
    prevbtn = async () => {
        console.log("prev bbtn")
        console.log("prev bcounttt",count)
        setCount(count - 10)
        console.log("prev bcounttt",count)
    ...
    tickclick = async () => {
        setProgress(true);
        setTitle('sucesso')
        setMessage('Agendamendo ja foi recepcionado.')
        AsyncStorage.setItem('selectAgendamentos', '')
        console.log("tick",setselectdata)
    ...
    cancelSchedule = async () => {
        ...
        console.log("tick",setselectdata)
        console.log("tick",setselectdata[20])
        console.log("date",orgdate);
        console.log("token",token);
        console.log("emitentFull",emitentFull.cnpj);
